...
class Loader extends AbstractHelper implements \Zend\ServiceManager\ServiceLocatorAwareInterface {
    ...
    public function loadEntities($entity_type_id, $store_id, $searchData, $attribute_codes, $searchType=array(), $options=array())
    {
        ...
        $entity_type_str = $this->getServiceLocator()->get('entityConfigService')->parseEntityTypeReverse($entity_type_id);

        $attributes = array();
        foreach($attribute_codes as $attId){
            $d = $this->getAttribute($attId, $entity_type_id);
            if(!$d || !(is_array($d) || $d instanceof \ArrayObject)){
                $this->getServiceLocator()->get('logService')->log(\Log\Service\LogService::LEVEL_DEBUG, 'load_noatt', 'loadEntities - failed to find attribute ' . $attId . ' for ' . $entity_type_id . '', array('res'=>$d, 'debug'=>$this->getAttributeDebuggingData(), 'codes'=>$attribute_codes));
                throw new NodeException('Could not find attribute ' . $attId . ' for ' . $entity_type_id);
            }
        ...
        $locateSelect = $this->getLocateSelect($entity_type_id, $store_id, $searchData, $searchType, $options);

        $this->getServiceLocator()->get('logService')->log(\Log\Service\LogService::LEVEL_DEBUGEXTRA, 'load_locate', 'loadEntities - locate query:' . $locateSelect->getSqlString($this->getAdapter()->getPlatform()), array('query'=>$locateSelect->getSqlString($this->getAdapter()->getPlatform())));
        ...
        if(count($result) == 0){
            ...
            $this->getServiceLocator()->get('logService')->log(\Log\Service\LogService::LEVEL_DEBUGEXTRA, 'load_locate_none', 'loadEntities - locate query found no results', array());
            return array();
        }
        $this->getServiceLocator()->get('logService')->log(\Log\Service\LogService::LEVEL_DEBUGEXTRA, 'load_locate_c', 'loadEntities - locate query found ' . count($result) . ' results', array('count'=>count($result)));
        ...
        if(count($attributes)){
            ...
            $this->getServiceLocator()->get('logService')->log(\Log\Service\LogService::LEVEL_DEBUGEXTRA, 'load_locate_fill', 'loadEntities - fill query: ' . $fillSql, array('sql'=>$fillSql));
            $fillResult = $this->getAdapter()->query($fillSql, \Zend\Db\Adapter\Adapter::QUERY_MODE_EXECUTE);

            $this->getServiceLocator()->get('logService')->log(\Log\Service\LogService::LEVEL_DEBUGEXTRA, 'load_locate_fillc', 'loadEntities - fill query got ' . count($fillResult) . ' values', array('count'=>count($fillResult)));
            ...
            $this->getServiceLocator()->get('logService')->log(\Log\Service\LogService::LEVEL_DEBUGEXTRA, 'load_locate_nf', 'loadEntities - no fill query, no attributes ', array());
        }
    ...
    public function enhanceEntity ( \Entity\Entity &$entity, $attribute_codes ){
        ...
        if(strlen($fillSql)){
            ...
            $this->getServiceLocator()->get('logService')->log(\Log\Service\LogService::LEVEL_DEBUGEXTRA, 'enhance_empty', 'enhanceEntity - no fill query for ' . $entity->getId() . ' with req ' . implode(', ', $attribute_codes), array('atts'=>$attribute_codes));
        }
    ...
    protected function processFillResult($fillResult){
        ...
        foreach($fillResult as $row){
            ...
            }else if(array_key_exists($row['att_id'], $entityValues[$row['ent_id']])){
     ...
     */
    protected function getLocateSelect( $entity_type_id, $store_id, $searchData, $searchType, $options ) {
        ...
        if(array_key_exists('fkey', $options)){
            foreach($options['fkey'] as $fkey_field=>$fkey_type){
                if(!array_key_exists($fkey_field, $searchData) && !in_array($fkey_field, $extraJoin)){
                    $this->addJoin($select, $fkey_field, true, $entity_type_id, $prefix);
                }
        ...
        }else if(!array_key_exists('no_select', $options)){
        ...
        }else if(array_key_exists('LOCAL_ID', $searchData)){
            throw new NodeException('Local ID specified but not checking node linkage');
        }

        if($entity_type_id !== false){
            $select->where(array($prefix.'e.type_id'=>$entity_type_id));
        }
        if($store_id > 0 && !array_key_exists('STORE_ID', $searchData)){
            $select->where(array($prefix.'e.store_id'=>$store_id));
        }
        ...
        foreach($searchData as $k=>$v){
            if(strpos($k, '.') !== false){
                ...
                if(!in_array($fkey_field, $resolved_fkeys)){
                    $resolved_fkeys[] = $fkey_field;
                    $d = $this->getAttribute($fkey_field, $entity_type_id);
                    if(!$d){
                    ...
                    }else if(!isset($d['fetch_data']['fkey_type'])){
                ...
                $attribute = $this->getAttribute($k, $entity_type_id);
                if(!$attribute){
                    throw new NodeException('Invalid search attribute ' . $k . ' for type ' . $entity_type_id);
                }
                $table = $prefix.'val_'.$k;
                $field = $table . '.value';
        ...
        foreach($extraJoin as $att){
            if(!array_key_exists($att, $searchData)){
                ...
                $this->addJoin($select, $att, $suppress, $entity_type_id, $prefix);
            }
        }
        ...
        if(array_key_exists('order', $options)){
            ...
            foreach($options['order'] as $att => $order){
                ...
                }else if($att == 'UNIQUE_ID'){
                    $orders[$prefix.'e.unique_id'] = $order;
                }else if($att == 'UPDATED_AT'){
                    $orders[$prefix.'e.updated_at'] = $order;
                }else if($att == 'PARENT_ID'){
    ...
    protected function addJoin(\Zend\Db\Sql\Select &$select, $att, $suppress, $entity_type_id, $prefix){
        // Need to join
        $attribute = $this->getAttribute($att, $entity_type_id);
    ...
    protected function addFkeyJoin(\Zend\Db\Sql\Select &$select, array &$fkey_type_ids, $fkey_field, $fkey_type, $prefix){
        ...
        }else if(strpos($fkey_field, '.') !== false){
        ...
        foreach($searchData as $k=>$v){
            if(!array_key_exists($k, $searchType)){
                if(is_array($v)){
                    ...
                    $searchType[$k] = 'eq';
                }else if(is_object($v) && $v instanceof \Entity\Entity){
                    $searchType[$k] = 'eq';
                }else{
